...
vec3 phongShading(vec3 normal, vec3 lightDir, vec3 viewDir, vec3 color, float shininess) {
    ...
    vec3 specularColor = specular * vec3(1.0); // Specular color is white
...
void initializeSpheres(out vec3 positions[7], out float radii[7], out vec3 colors[7], out float shininess[7]) {
    setupSphere(positions[0], radii[0], colors[0], shininess[0], vec3(-0.09, 0.76, -0.3), 0.26, vec3(0.063, 0.322, 0.953), 20.0); // Medium Blue
    setupSphere(positions[1], radii[1], colors[1], shininess[1], vec3(-0.49, 0.60, -0.5), 0.27, vec3(0.337, 0.196, 0.655), 18.0); // Purple
    setupSphere(positions[2], radii[2], colors[2], shininess[2], vec3(-0.35, 0.28, -0.9), 0.32, vec3(0.584, 0.196, 0.588), 22.0); // Magenta
    setupSphere(positions[3], radii[3], colors[3], shininess[3], vec3(0.22, 0.45, 0.05), 0.19, vec3(0.059, 0.365, 0.696), 22.0);  // Teal Blue
    setupSphere(positions[4], radii[4], colors[4], shininess[4], vec3(0.3, 0.13, 0.05), 0.18, vec3(0.063, 0.533, 0.541), 22.0);   // Cyan
    setupSphere(positions[5], radii[5], colors[5], shininess[5], vec3(0.2, -0.21, -0.06), 0.23, vec3(0.0, 0.3, 0.0), 22.0);       // Dark Green
    setupSphere(positions[6], radii[6], colors[6], shininess[6], vec3(-0.15, -0.25, 0.1), 0.20, vec3(0.055, 0.741, 0.059), 22.0); // Bright Green
}
